...
describe('System Logic Refinement Suite: Time Overlap, Asia/Saigon Timezone & Attendance Roster', () => {
  it('1. ConflictEngine: Phát hiện trùng lịch khi 2 ca giao nhau về giờ thực tế (18:00 - 20:00 và 19:00 - 21:00)', async () => {
    const engine = new ConflictEngine(repo);
    const testDate = '2026-11-25';

    // Tạo slot 1: 18:00 - 20:00 cho GV001 tại phòng P.101
    ...
    // Thử xếp slot 2: 19:00 - 21:00 (giao nhau từ 19:00 đến 20:00) cho GV001
    const conflictResult = await engine.checkScheduleConflict({
      classId: 'CLS02',
      teacherId: 'GV001',
      roomId: 'P.202', // khác phòng
      date: testDate,
      shiftId: 2,      // khác shiftId
      startTime: '19:00',
      endTime: '21:00',
      subject: 'Lý',
      status: 'Đã lên lịch'
    ...
    assert.equal(conflictResult.hasConflict, true, 'Phải phát hiện xung đột trùng giáo viên khi giờ bị giao nhau');
    assert.equal(conflictResult.conflicts[0].type, 'TEACHER_CONFLICT');
  });
